<script>
    var teacher_cal_lock = false;
    $(function () {

        $('#calendar-teacher-settings-toggle').click(function () {
            $('#calendar-teacher-settings').toggle('slow');
        });

        $('#calendar-teacher-legend-toggle').click(function () {
            $('#calendar-teacher-legend-modal').modal();
        });
        ...
        $("#calendar-teacher").zabuto_calendar({
            ...
            data: [
                    @if($teacher->nearest_meeting)
                {
                    ...
                    @endif
                    @foreach($teacher->classes_all as $tc)
                    @if( (\Illuminate\Support\Facades\Auth::user()->hasRole('admin') or \Illuminate\Support\Facades\Auth::id() == $teacher->inst->id)
                        and (($tc->is_past() and count($tc->students) > 0)
                        or (!$tc->is_past() and (count($tc->students) > 0 or $tc->canceled == 1))))
                {
                    date: '{{ $tc->class_date }}',
                    classname: @if($tc->is_past()) "event-past-class" @elseif($tc->canceled) "event-taken-classes" @else "event-taken-classes-teacher" @endif ,
                },
                @endif
                @endforeach
            ]
        ...
        function myDateFunction(id, fromModal) {
            ...
            let templates = $("#eventTeacherModalTemplates");
            ...
            if (!hasEvent || teacher_cal_lock) {
                return false;
            }

            teacher_cal_lock = true;
            ...
            $.ajax({
                ...
                success: function (response) {
                            ...
                            if (this.classes.length > 0) {
                                ...
                                $.each(this.classes, function () {
                                    ...
                                    if (this.class_instance) { //already existing class
                                        ...
                                        tmp_avail.find("#class_time").html(this.th.class_start.substr(0, 5) + " - " + this.th.class_end.substr(0, 5));
                                        link = link.substr(0, link.length - 1) + this.th.id + "/preview/" + date;
                                    }
                    ...
                    $.each(JSON.parse(response.enrolled), function () {
                        ...
                        tmp.find("#teacher_image").attr('src', this.teacher_img);
